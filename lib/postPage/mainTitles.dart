...
class MainTitles extends StatefulWidget {
  ...
  @override
  _MainTitlesState createState() => _MainTitlesState();
}
...
class _MainTitlesState extends State<MainTitles> {
                            ...
                            child: ListTile(
                              onTap: (){
                                Navigator.push(
                                  context,
                                  MaterialPageRoute(
                                    builder: (context) => SubTitles(index:index),
                                  ),
                                );
                              },
                                ...
                                children: [
                                  ...
                                  Visibility(
                                    ...
                                    child: IconButton(onPressed: () async {
                                      await showAlertDialogUpdate(context, index, array2[index]["baslik"]);
                                      array2 = snapshot.data;
  ...
  showAlertDialogDelete(BuildContext context, int index) {
    ...
    Widget continueButton = TextButton(
      ...
      onPressed: () async {
        await FirebaseService().deleteMainTitle(index);
        Navigator.of(context).pop();
        setState(() {
        });
      },
  ...
  showAlertDialogUpdate(BuildContext context, int index, String title) {
    ...
    Widget continueButton = TextButton(
      child: Text("Devam Et", style: TextStyle(color: Colors.red)),
      onPressed: () async {
        await FirebaseService().updateMainTitle(index, title);
        Navigator.of(context).pop();
        setState(() {
        });
      },
